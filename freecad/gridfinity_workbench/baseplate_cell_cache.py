...
def set_cell_cache_max_entries(max_entries: int) -> None:
    ...
    global _CELL_CACHE_MAX  # noqa: PLW0603
...
def normalize(value: object) -> object:  # noqa: PLR0911
    """Normalize a value to a JSON-serializable form for caching."""
...
def make_key(payload: dict) -> str:
    """Build a cache key from a payload dictionary."""
...
def get_or_build(key: str, build_fn: Callable[[], Part.Shape]) -> Part.Shape:
    """Get shape from cache or build and cache it."""
    if _CELL_CACHE_MAX <= 0:
        return build_fn()
